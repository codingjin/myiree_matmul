...
TiledMatrix* pack_matrix_p(const float* src, int rows, int cols, int tile_size_m, int tile_size_n);
void unpack_matrix_p(float* dst, const TiledMatrix* tiled, int tile_size_m, int tile_size_n);
...
void iree_mmt4d_p(TiledMatrix* acc, const TiledMatrix* lhs, const TiledMatrix* rhs);
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < WARMUP; ++i) {
        // Pack matrices into tiled format
        memset(C_iree, 0, M * N * sizeof(float));
        TiledMatrix* A_tiled = pack_matrix_p(A, M, K, TILE_M, TILE_K);
        TiledMatrix* B_tiled = pack_matrix_p(B, K, N, TILE_K, TILE_N);
        TiledMatrix* C_tiled = pack_matrix_p(C_iree, M, N, TILE_M, TILE_N);
        iree_mmt4d_p(C_tiled, A_tiled, B_tiled);
        unpack_matrix_p(C_iree, C_tiled, TILE_M, TILE_N);
        free(A_tiled->data);
        free(A_tiled);
        free(B_tiled->data);
        free(B_tiled);
